...
public class AEAutomationScenes {
    public static void craftingCPU(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.overlay().showOutlineWithText(sel.fromTo(1, 1, 6, 2, 2, 7), 40)
        ...
        scene.idle(20);
        scene.overlay().showOutlineWithText(sel.fromTo(4, 1, 6, 5, 2, 7), 40)
    ...
    public static void interfaceBasic(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.overlay().showOutlineWithText(sel.position(3, 1, 2), 40)
                .colored(PonderPalette.GREEN)
        ...
        scene.overlay()
                .showControls(vec.of(3.5, 2, 2.5), Pointing.DOWN, 40)
                .withItem(item);
        scene.idle(40);
        scene.world().showSection(sel.position(3, 1, 1), Direction.SOUTH);
        scene.idle(20);
        scene.text(60, "It will fill itself with items to the amount you set")
                .pointAt(vec.of(3.5, 2, 2.5))
                .placeNearTarget();
    ...
    public static void annihilationPlane(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.text(60, "If the network can still store items, the Annihilation Plane will collect blocks or item entities in front of it into the network");
        scene.idle(60);
        scene.world().setBlocks(sel.position(2, 2, 1),
    ...
    public static void annihilationPlaneFilter(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world().destroyBlock(pos.at(2, 2, 1));
        scene.idle(40);
        scene.overlay().showOutlineWithText(sel.position(2, 2, 1), 60)
                .colored(PonderPalette.RED)
        ...
        scene.overlay().showOutline(PonderPalette.GREEN, new Object(),
                sel.position(4, 1, 2), 60);
        scene.idle(60);
        scene.text(50, "Set up the entire production as a sub-network without placing any drives")
                .pointAt(point);
        scene.idle(60);
        ...
        scene.world().destroyBlock(pos.at(2, 2, 1));
        scene.overlay().showControls(vec.of(5.5, 2, 2.5), Pointing.DOWN, 40)
                .withItem(crystal);
        scene.idle(60);
        scene.text(60, "It is important to note that blocks without drops will be destroyed by the Annihilation Plane regardless");
        scene.idle(80);
    ...
    public static void formationPlane(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.text(60, "Formation Planes can output items from the network as either blocks or item entities");
        scene.idle(80);
        ...
        scene.overlay().showOutlineWithText(sel.position(2, 1, 1), 60)
                ...
                .pointAt(vec.of(2.5, 1.5, 1.5));
        scene.idle(80);
        scene.text(60, "Configuring the filter options of the Formation Plane allows it to only output items within the filter")
                .attachKeyFrame();
        scene.idle(80);
    ...
    public static void molecularAssembler(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.text(60, "You need to place it together with the Pattern Provider")
                ...
                .attachKeyFrame();
        scene.idle(80);
        scene.text(60, "Molecular Assembler can automate crafting, stonecutting, and smithing table recipes, sending the products back to the Pattern Provider");
        scene.idle(80);
        ...
        scene.idle(80);
        scene.text(60, "Later on, you can replace the Molecular Assembler with the more efficient Assembler Matrix");
        scene.idle(80);
    }
    ...
    public static void craftingPrinciple(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.text(40, "This is a Pattern Provider")
                .pointAt(vec.of(2.5, 2.5, 3.5)).attachKeyFrame();
        scene.idle(60);
        scene.overlay().showControls(vec.of(2.5, 3, 3.5), Pointing.DOWN, 20)
                .rightClick()
        ...
        scene.idle(20);
        scene.text(120, "Thus you need to build a return system, or you can also use other Interfaces or Providers to return items to the network");
        scene.idle(120);
    }
    ...
    public static void craftingParallel(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(40);
        scene.text(60, "Multiple Pattern Providers executing the same pattern can achieve parallelism across multiple machines")
                .attachKeyFrame();
        scene.idle(80);
        scene.text(60, "Memory Cards can copy machine configurations and apply them to other machines, and Pattern Providers are no exception.");
        scene.idle(80);
        scene.text(60, "Use the Memory Card to right-click the Pattern Provider while sneaking...");
        scene.idle(20);
        scene.overlay().showControls(vec.of(1.5, 2, 3.5), Pointing.DOWN, 40)
                .whileSneaking()
                .withItem(memoryCard);
        scene.idle(60);
        ...
        scene.text(60, "Then, use the Memory Card to right-click the target Pattern Provider...")
                .attachKeyFrame();
        scene.idle(40);
        scene.overlay().showControls(vec.of(2.5, 2, 3.5), Pointing.DOWN, 40)
                .withItem(memoryCard);
        scene.idle(20);
        scene.world().modifyBlock(
                pos.at(2, 1, 3),
        ...
        scene.idle(40);
        scene.text(40, "The Memory Card will consume a Blank Pattern from your inventory and copy the pattern into the target Pattern Provider")
                .attachKeyFrame();
        scene.idle(60);
        ...
        for (int index = 1; index <= 5; index++) {
            scene.world().showSection(
                    sel.fromTo(2 + index, 1, 3, 2 + index, 1, 5),
                    Direction.DOWN
            );
    ...
    public static void providerSubnetwork(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.rotateCameraY(90);
        scene.idle(20);
        scene.overlay().showOutlineWithText(sel.fromTo(1, 1, 1, 2, 1, 1), 40)
                .colored(PonderPalette.GREEN)
                .text("ME Interface is also an inventory, when the Pattern Provider is adjacent to an ME Interface...")
        ...
        scene.idle(80);
        scene.text(80, "This is important for some complex machine mechanisms, you can refer to the Metallurgic Infuser's ponder information.");
        scene.idle(80);
    }
}
