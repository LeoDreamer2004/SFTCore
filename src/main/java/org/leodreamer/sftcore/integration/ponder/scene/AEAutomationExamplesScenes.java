...
public class AEAutomationExamplesScenes {
    ...
    public static void metallurgicInfuser(SFTSceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.text(80, "We use an Interface to receive items from the Pattern Provider, forming a subnetwork locally")
        ...
        scene.overlay().showOutlineWithText(sel.position(2, 1, 3), 80)
                .colored(PonderPalette.GREEN)
                .text("Two Storage Buses represent different input slots, distributing items within the subnetwork")
                .attachKeyFrame();
        scene.idle(100);

        scene.rotateCameraY(90);
        scene.text(80, "Setting the output direction of the Pattern Provider to prevent it from distributing materials directly to the adjacent Metallurgic Infuser")
                .attachKeyFrame();
        scene.idle(100);
        ...
        scene.overlay()
                ...
                .rightClick();
        scene.world().modifyBlock(pos.at(1, 1, 2),
                (state) -> state.setValue(PatternProviderBlock.PUSH_DIRECTION, PushDirection.UP),
                false);
        scene.idle(60);
        scene.rotateCameraY(-90);

        scene.text(100, "Now, we need to program the Pattern Provider with the correct patterns for automation")
                .attachKeyFrame();
        scene.idle(40);
        scene.overlay()
        ...
        scene.text(40, "Open the configuration interface of the Metallurgic Infuser...")
                .pointAt(vec.of(2.5, 1.5, 2.5))
                .placeNearTarget();
        scene.idle(40);
        scene.overlay().showOutlineWithText(sel.position(2, 2, 2), 60)
                .colored(PonderPalette.GREEN)
        ...
        scene.overlay().showOutlineWithText(sel.position(1, 1, 2), 80)
        ...
        scene.text(80, "Encoding Pattern: 8 Iron Ingots + 1 Enriched Redstone = 8 Infused Alloys")
                .pointAt(vec.of(2, 1, 2.5))
                .attachKeyFrame();
        scene.overlay().showControls(vec.of(2, 1.5, 2), Pointing.DOWN, 80)
                .withItem(AEItems.BLANK_PATTERN.stack());
        scene.idle(100);

        scene.text(80, "With that, the automation of the Metallurgic Infuser is complete! Note that for different recipes, you need to mark the Storage Bus filter as needed");
        scene.idle(80);
    }
}
